...
<body>
    ...
    {{-- ===== Rincian ===== --}}
    <div class="section-title">Rincian Gaji ({{ count($rows) }})</div>
    <table class="tx">
        <thead>
            <tr>
                ...
                <th class="num" style="width: 14%;">Gaji Pokok</th>
                <th class="center" style="width: 10%;">Status</th>
                <th class="num lastcol" style="width: 14%;">Total Gaji</th>
            </tr>
        </thead>
        <tbody>
            @forelse($rows as $i => $r)
            <tr class="{{ $i % 2 === 1 ? 'alt' : '' }}">
                ...
                <td class="center">
                    <span class="pill {{ $r['status'] === 'completed' ? 's-completed' : 's-pending' }}">{{ ucfirst($r['status']) }}</span>
                </td>
                <td class="num t-money">{{ $rupiah($r['total']) }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="8" class="empty">Tidak ada data gaji untuk kriteria ini.</td>
            </tr>
            @endforelse
        </tbody>
        @if(count($rows) > 0)
        <tfoot>
            <tr>
                <td colspan="7" class="num">Total Keseluruhan Gaji</td>
                <td class="num t-money">{{ $rupiah($summary['total']) }}</td>
            </tr>
        </tfoot>
        @endif
    </table>

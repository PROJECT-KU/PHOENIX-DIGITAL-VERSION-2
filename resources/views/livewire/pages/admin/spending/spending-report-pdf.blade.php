...
<head>
    ...
    <style>
        * { font-family: 'DejaVu Sans', sans-serif; margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .p-akun { background: #eff6ff; color: #2563eb; }
        .p-lain { background: #f5f3ff; color: {{ $accent }}; }
        ...
        <tr>
            <td>
                <div class="scard" style="background: #f5f3ff; border-color: #e0e7ff;">
                    ...
                    <div class="value t-ex">{{ $rupiah($summary['total']) }}</div>
                </div>
            </td>
            <td>
            ...
            <td>
                <div class="scard" style="background: #eff6ff; border-color: #dbeafe;">
                    <div class="label">Pembelian Akun</div>
                    <div class="value" style="color: #2563eb;">{{ $rupiah($summary['pembelian_akun']) }}</div>
                </div>
            </td>
            <td>
                <div class="scard" style="background: #eef2ff; border-color: #e0e7ff;">
                    <div class="label">Pengeluaran Lainnya</div>
                    <div class="value" style="color: {{ $accent }};">{{ $rupiah($summary['lainnya']) }}</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="tx">
        <thead>
            <tr>
                ...
                <th style="width: 21%;">Deskripsi</th>
                <th class="center" style="width: 9%;">Status</th>
                <th style="width: 11%;">Penginput</th>
                <th style="width: 11%;">PIC Pembeli</th>
                <th class="num lastcol" style="width: 12%;">Nominal</th>
            </tr>
        </thead>
        <tbody>
            @forelse($rows as $i => $r)
            <tr class="{{ $i % 2 === 1 ? 'alt' : '' }}">
                ...
                <td>{{ $r['tanggal'] }}</td>
                <td><span class="pill {{ $r['is_akun'] ? 'p-akun' : 'p-lain' }}">{{ $r['jenis'] }}</span></td>
                <td>{{ $r['deskripsi'] }}</td>
                <td class="center">
                    <span class="pill {{ strtolower($r['status']) === 'completed' ? 's-completed' : 's-pending' }}">{{ $r['status'] }}</span>
                </td>
                <td>{{ $r['penginput'] }}</td>
                <td>{{ $r['pic'] }}</td>
                <td class="num t-ex">{{ $rupiah($r['nominal']) }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="9" class="empty">Tidak ada data pengeluaran untuk kriteria ini.</td>
            </tr>
            @endforelse
        </tbody>
        @if(count($rows) > 0)
        <tfoot>
            <tr>
                <td colspan="8" class="num">Total Pengeluaran</td>
                <td class="num t-ex">{{ $rupiah($summary['total']) }}</td>
            </tr>
        </tfoot>
        @endif
    </table>

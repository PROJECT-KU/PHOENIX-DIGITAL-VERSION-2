...
<div>
            ...
            <div class="card-body p-3 p-lg-4">
                <div class="row g-2 align-items-center">
                    <div class="col-12 col-lg-6">
                        {{-- Kotak cari bersama, berikut tombol pengosongnya. Sebelumnya
                             ditulis sendiri di sini tanpa tombol silang, sehingga satu-satunya
                             cara mengembalikan daftar utuh adalah menghapus ketikannya
                             huruf demi huruf. --}}
                        @include('livewire.pages.admin.orcha.partials.cari', [
                            'petunjuk' => 'Cari ' . strtolower($judul) . '...',
                        ])
                    </div>
                ...
                <div class="col-12 col-md-6 col-xl-4" wire:key="{{ $jenis }}-{{ $baris['id'] }}">
                    <div class="card orcha-kartu orcha-etalase h-100">
                        ...
                        {{-- Kotak gambarnya SELALU ada, juga ketika fotonya belum
                             ...
                             tetangganya. --}}
                        {{-- Partner memakai perlakuan gambar yang SAMA dengan testimoni
                             dan destinasi: memenuhi kotak, dipotong seperlunya.

                             Sebelumnya partner punya varian sendiri yang memuat
                             gambarnya utuh di atas latar putih, dengan alasan logo lebar
                             tidak boleh kehilangan tulisannya. Alasannya masih benar
                             untuk logo — tetapi yang diunggah ke sini pada praktiknya
                             poster dan foto, dan yang utuh di antara yang terpotong
                             membuat sebaris kartu terlihat tidak sejenis.

                             Konsekuensinya diterima sadar: logo yang benar-benar lebar
                             akan terpotong di sini. --}}
                        <div class="orcha-etalase-foto {{ $foto ? '' : 'tanpa' }}">
                            @if ($foto)
                        ...
                        <div class="card-body p-3 p-lg-4 d-flex flex-column">
                            ...
                            @php
                                /*
                                 | Partner TIDAK punya keterangan — di mana pun.
                                 |
                                 | Tidak di tabelnya (hanya nama dan foto), tidak di
                                 | balasan API-nya, dan tidak di formulirnya. Kartunya
                                 | dulu ikut membaca 'deskripsi' yang tidak pernah ada,
                                 | sehingga TIAP partner selamanya bertuliskan "Keterangan
                                 | belum ditulis." — mengumumkan kekurangan yang tidak
                                 | punya tempat untuk diperbaiki, dan admin mencari-cari
                                 | isian yang memang tidak pernah dibuat.
                                 */
                                $punyaKeterangan = $jenis !== 'partner';
                    ...
                    <form wire:submit="simpan">
                        <div class="modal-body">
                            ...
                            <div class="orcha-foto-kotak @error('gambar') galat @enderror">
                                {{-- Pratinjaunya berganti pemintal selama berkasnya naik.

                                     Unggahan gambar 4 MB lewat sambungan lambat bisa
                                     memakan belasan detik, dan selama itu kotak pratinjau
                                     diam memperlihatkan gambar LAMA — atau kotak kosong.
                                     Yang memilih berkas menyangka pilihannya tidak masuk
                                     lalu memilih ulang, dan unggahan yang sedang jalan
                                     dibuang dari awal.

                                     Menyasar `gambar` secara khusus: tanpa itu pratinjaunya
                                ...
                                <div class="orcha-foto-rupa">
                                    ...
                                    {{-- Pemintal DITUMPANGKAN di atas pratinjaunya, bukan
                                         menggantikannya.

                                         Ditumpangkan karena dua sebab. Pertama, gambar lama
                                         tetap terlihat samar di baliknya, jadi jelas bahwa
                                         yang sedang naik ini penggantinya. Kedua — dan ini
                                         yang menentukan bentuknya — elemen yang ditukar
                                         butuh display:flex saat tampil, dan aturan itu akan
                                         mengalahkan penyembunyi [wire:loading] di lembar gaya
                                         bersama, sehingga pemintalnya tergambar sebelum
                                         unggahan dimulai. Yang ditumpangkan cukup diletakkan
                                         dengan position, tanpa menyentuh display sama sekali. --}}
                                    <span wire:loading wire:target="gambar" class="orcha-foto-naik">
                        ...
                        {{-- Sekeluarga dengan tombol di halaman formulirnya: tombol Orcha
                             setinggi 38px, bukan tombol Bootstrap polos yang bentuk dan
                             sudutnya lain sendiri. --}}
                        <div class="modal-footer border-0">
                            <button type="button" class="orcha-btn orcha-btn-lembut orcha-btn-besar"
                                wire:click="tutup">
                                <i class="bi bi-x-lg"></i> Batal
                            </button>
                            {{-- Tombolnya menunggu DUA hal: unggahan gambar dan
                                 penyimpanannya.

                                 Menekan Simpan selagi gambarnya masih naik mengirim
                                 testimoni tanpa foto — Livewire belum memegang berkasnya —
                                 dan admin baru sadar setelah kartunya tampil kosong. Jadi
                                 tombolnya ikut mati dan memintal selama unggahan.

                                 Kalimatnya dibedakan: yang menunggu unggahan diberi tahu
                                 bahwa yang lama itu gambarnya, bukan simpanannya. --}}
                            <button type="submit" class="orcha-btn orcha-btn-utama orcha-btn-besar"
                                wire:loading.attr="disabled" wire:target="simpan,gambar">
                                <span wire:loading.remove wire:target="simpan,gambar">
                                    <i class="bi bi-save"></i> Simpan
                                </span>
                                <span wire:loading wire:target="gambar">
                                    <span class="spinner-border spinner-border-sm me-2"
                                        role="status" aria-hidden="true"></span>Mengunggah gambar…
                                </span>
    ...
    <style>
        ...
        /* Varian .logo untuk partner DIHAPUS, bukan ditinggalkan menganggur.

           Dulu ia memuat logo partner utuh di atas latar putih supaya logo
           lebar tidak kehilangan tulisannya. Alasannya masih benar untuk logo,
           tetapi kartunya kini disamakan dengan testimoni dan destinasi —
           memenuhi kotak, dipotong seperlunya. Gaya yang tidak lagi menempel
           pada apa pun akan dipakai ulang keliru oleh yang berikutnya mencarinya. */

...
/* ---------- GAMBAR TAMBAHAN DESTINASI ---------- */

function fakeDestinasi(array $baris = []): void
{
    config()->set('orcha.url', 'https://orcha.test/api/v1');
    config()->set('orcha.kunci', 'kunci-uji');
    cache()->forget('orcha.rujukan');
    ...
    Http::fake([
        ...
        '*' => Http::response(['data' => [array_merge([
            'id' => 3, 'nama' => 'Bromo', 'wilayah' => 'jawa', 'wilayah_label' => 'Jawa',
            'provinsi' => 'Jawa Timur', 'deskripsi' => 'Lautan pasir', 'total_pengunjung' => 26700,
            'foto' => '/storage/destinasi/utama.webp',
            'sub_foto' => ['/storage/destinasi/tambahan/a.webp', '/storage/destinasi/tambahan/b.webp'],
            'batas_sub_foto' => 3,
        ], $baris)], 'meta' => ['halaman' => 1, 'halaman_terakhir' => 1, 'total' => 1]]),
    ]);
}

test('mengubah destinasi memuat gambar tambahannya', function () {
    fakeDestinasi();

    // Tanpa ini admin yang mengurus destinasi dari lemon tidak punya cara
    // melihat gambar tambahan, apalagi mengubahnya — harus membuka admin
    // bawaan Orcha, yang justru dihindari dengan adanya halaman ini.
    Livewire::actingAs(adminOrcha())->test(OrchaEtalaseList::class)
        ->call('ubah', ['id' => 3, 'nama' => 'Bromo', 'wilayah' => 'jawa',
            'sub_foto' => ['/storage/destinasi/tambahan/a.webp'], 'batas_sub_foto' => 3])
        ->assertSet('subFotoTetap', ['/storage/destinasi/tambahan/a.webp'])
...
test('menghapus satu gambar tambahan hanya mengeluarkannya dari daftar', function () {
    ...
    // Berkasnya baru dibuang di Orcha saat disimpan, jadi menutup jendela tanpa
...
test('daftar gambar yang dipertahankan ikut terkirim saat menyimpan', function () {
    ...
    Livewire::actingAs(adminOrcha())->test(OrchaEtalaseList::class)
        ->call('ubah', ['id' => 3, 'nama' => 'Bromo', 'wilayah' => 'jawa',
            'sub_foto' => ['/storage/destinasi/tambahan/a.webp']])
        ->call('hapusSubFoto', '/storage/destinasi/tambahan/a.webp')
        ->call('simpan')
        ->assertHasNoErrors();
...
test('batas gambar tambahan dijaga sebelum dikirim', function () {
    ...
    // Menyerahkan penjagaannya ke Orcha berarti admin baru tahu setelah
    // menunggu unggahan selesai — padahal jumlahnya sudah bisa dihitung di sini.
    Livewire::actingAs(adminOrcha())->test(OrchaEtalaseList::class)
        ->call('ubah', ['id' => 3, 'nama' => 'Bromo', 'wilayah' => 'jawa',
            'sub_foto' => ['/storage/a.webp', '/storage/b.webp', '/storage/c.webp'],
            'batas_sub_foto' => 3])
        ->set('subFoto', [Illuminate\Http\UploadedFile::fake()->image('d.jpg')])
        ->call('simpan')
...
test('sisa tempat menghitung yang tersimpan dan yang baru dipilih', function () {
    ...
    $uji = Livewire::actingAs(adminOrcha())->test(OrchaEtalaseList::class)
        ->call('ubah', ['id' => 3, 'nama' => 'Bromo', 'wilayah' => 'jawa',
            'sub_foto' => ['/storage/a.webp'], 'batas_sub_foto' => 3]);

    expect($uji->instance()->sisaSubFoto())->toBe(2);

    $uji->set('subFoto', [Illuminate\Http\UploadedFile::fake()->image('b.jpg')]);

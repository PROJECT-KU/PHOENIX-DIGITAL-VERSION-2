...
/**
 * Lembar serah terima satu unit sewa.
 *
 * Halaman tersendiri, bukan jendela di atas daftar. Isinya panjang — tenggat,
 * kilometer, bahan bakar, jaminan, pemeriksaan bodi per bagian, dan rincian
 * denda — dan jendela setinggi layar penuh menyulitkan admin membandingkan
 * kondisi awal dengan akhir sambil menggulung.
 *
 * Dibuka lewat alamat biasa, jadi tombol di daftar cukup berupa tautan. Tidak
 * ada yang perlu berhasil lebih dulu sebelum halamannya terbuka.
 */
class OrchaSerahTerimaForm extends Component
{
    use BagianUnit;
    use MemanggilOrcha;
    use WithFileUploads;
    ...
    /** Penyewaan yang lembarnya sedang dibuka. */
    ...
    public function mount(int $penyewaan): void
    {
        /*
         ...
         |
         | Lembarnya panjang — tenggat, kilometer, bahan bakar, jaminan,
         | pemeriksaan bodi, dan rincian denda — dan jendela yang isinya
         | segulung layar penuh menyulitkan admin membandingkan kondisi awal
         | dengan akhir. Ia juga dibuka lewat alamat biasa, jadi tombolnya cukup
         | tautan: tidak ada yang bisa gagal sebelum halamannya terbuka.
         */
        $this->serahTerimaUntuk = $penyewaan;

        $baris = $this->muat("/penyewaan/{$penyewaan}")['data'] ?? [];

        if ($baris === []) {
            $this->galat = $this->galat ?: 'Data sewa itu tidak ditemukan di Orcha.';
    ...
    {
        $this->redirectRoute('admin.orcha.penyewaan', navigate: true);
    }
    ...
    public function bagianPeriksa(): array
    {
        return $this->bagianUntukUnit(
            data_get($this->sewa, 'kendaraan.jenis'),
            $this->kondisiAwal,
            $this->kondisiAkhir,
        );
    }
    ...
    {
        return view('livewire.pages.admin.orcha.penyewaan.serah-terima', [
            'bagianPeriksa' => $this->bagianPeriksa(),
